...
@Component({
  ...
  styleUrl: './general-dialog.component.css'
})
...
groupData(): void {
  ...
  openDialog() {
    ...
    dialogConfig.width = '80%';
    dialogConfig.height = '80%';
    this.dialog.open(GeneralDialogComponent , dialogConfig);
  }
    ...
    this.http.post<any>(url, formData).subscribe(
      response => {
        ...
        if (response.filtered_data) {
          ...
          if (this.data.uniqueValues && this.data.uniqueValues.created_at_date) {
            const filteredDates = this.filteredData.map((item: any) => item.created_at_date);
            const uniqueDates = this.data.uniqueValues.created_at_date;
            ...
            console.error('uniqueValues does not contain created_at_date:', this.data.uniqueValues);
            this.groupedData = [];
            this.filteredData = [];
            this.updateTableHeaders();
            this.dataSource.data = this.filteredData;
 ...
 downloadGroupedData(): void {
   ...
   try {
     ...
     const title = 'Genearal Master Custom Report';
